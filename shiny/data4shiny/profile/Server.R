...
shinyServer(function(input, output) {
  ...
  output$view <- renderPlot({
    ...
    if(is.null(myGene) ) {
      ...
      #pour le moment on ne gère pas les genes présent plusieurs fois sur la puce
      # on prend la première probe trouvée
      warning("Ce gène contient plusieurs probes : seule la première est représentée")
      myGene = myGene[1]
      mat = data.frame(t(datasetInput()[myGene,-1]), colnames(datasetInput())[-1], phenoInput()[,-1])
      colnames(mat) = c("int","ID",colnames(phenoInput())[-1])
      p= ggplot2_barplot(mat,input$phenotype)
