...
declare_contrastes <- function(y){
  ...
  contr <- apply( MC, 2, function(v) if(all(v==0)) {
    return(character(0)) 
  } else if ( (nlev==2) & !identical(names(table(v)), c("-1","1")) ) {
    stop("Utiliser seulement -1 ou 1 pour declarer des contrastes.")
  }else if ( (nlev > 2) & !identical(names(table(v)), c("-1","0","1")) ) {
    stop("Utiliser seulement -1, 0 ou 1 pour declarer des contrastes.")
...
declare_contrastes_2 <- function(y){
